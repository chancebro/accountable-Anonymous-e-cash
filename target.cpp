...
pk_3_proof pk_3_prove(User &user_out,const mcl::bn::G1 g, const mcl::bn::G1 &g0, const mcl::bn::G1 g1,const mcl::bn::G2 g_G2, const mcl::bn::Fp12 &G,std::string INFO) {
    ...
    Fp12::pow(proof.M,G_t_info,user_out.u);//EXP1
    Fp12::pow(user_out.M,G_t_info,user_out.u);//EXP2

    G1::mul(g0r1,g0,r1);//MP2
    G1::mul(g1r2,g1,r2);//MP3
    G1::mul(g1r1,g1,r1);//MP4

    G1::add(proof.A1,g0r1,g1r2);
    G1::mul(proof.A1e,proof.A1,user_out.e);//MP5
    ...
    G1 g0r_r1,g1r_r2;
    G1::mul(g0r_r1,g0,r_r1);//MP6
    G1::mul(g1r_r2,g1,r_r2);//MP7
    G1::add(proof.Cm_A1,g0r_r1,g1r_r2);

    G1::mul(proof.Cm_A1e,proof.A1,r_e);//MP8
...
bool pk_3_verify(const pk_3_proof &proof, const mcl::bn::G1 g, const mcl::bn::G1 &g0, const mcl::bn::G1 g1,const mcl::bn::G2 g_G2, const mcl::bn::Fp12 &G) {
    ...
    Fp12::mul(result, e_A2w, inv_e_gg_G2);//MP6 // 최종 결과 계산
...
SPK_proof SPK_prove(User &user_out,const mcl::bn::G1 h, const mcl::bn::G1 &h0, const mcl::bn::G1 h1, const mcl::bn::G1 h2,const mcl::bn::G2 h_G2, const mcl::bn::Fp12 &G,const mcl::bn::Fp12 &H,const mcl::bn::Fp12 &H1,std::string INFO,pk_3_proof pk_3) {
    ...
    pairing(ef,proof.B2,h_G2);//BP1
    Fp12::pow(proof.Cm_ef,ef,r_f);//EX10
    pairing(er1,h1,X);//BP2
    Fp12::pow(proof.Cm_er1,er1,r_r1);//EX11
    pairing(ed1,h1,h_G2);//BP3
    Fp12::pow(proof.Cm_ed1,ed1,r_d1);//EX12
    pairing(et,h0,h_G2);//BP4
    Fp12::pow(proof.Cm_et,et,r_t);//EX13
    pairing(eu,h1,h_G2);//BP5
    Fp12::pow(proof.Cm_eu,eu,r_u);//EX14
    pairing(ev,h2,h_G2);//BP6
    Fp12::pow(proof.Cm_ev,ev,r_v);//EX5
...
bool SPK_verify(const SPK_proof &proof, const mcl::bn::G1& h, const mcl::bn::G1 &h0, const mcl::bn::G1& h1,const mcl::bn::G1& h2,const mcl::bn::G2 &h_G2,const mcl::bn::Fp12 &G, const mcl::bn::Fp12 &H,const mcl::bn::Fp12 &H1) {
    ...
    Fp12::mul(result, e_B2X, inv_e_hh_G2);//MP6 // 최종 결과 계산
    ...
    Fp12::mul(templ2,proof.Cm_ed1,templ1);//MP8
    Fp12::mul(templ3,proof.Cm_et,templ2);//MP9
    Fp12::mul(templ4,proof.Cm_eu,templ3);//MP10
    Fp12::mul(templ5,proof.Cm_ev,templ4);//MP11
    Fp12::mul(el,templ5,resultc);//MP12

    Fp12 tempr1,tempr2,tempr3,tempr4,tempr5,tempr6,inv_tempr1,er;

    Fp12 ef,er1,ed1,et,eu,ev;

    pairing(ef,proof.B2,h_G2);//BP3
    pairing(er1,h1,X);//BP4
    pairing(ed1,h1,h_G2);//BP5
    pairing(et,h0,h_G2);//BP6
    pairing(eu,h1,h_G2);//BP7
    pairing(ev,h2,h_G2);//BP8
    

    Fp12::pow(tempr1,ef,proof.Z_f);//EX2
    Fp12::inv(inv_tempr1, tempr1);   // 역원 계산
    
    Fp12::pow(tempr2,er1,proof.Z_r1);//EX3
    Fp12::pow(tempr3,ed1,proof.Z_d1);//EX4
    Fp12::pow(tempr4,et,proof.Z_t);//EX5
    Fp12::pow(tempr5,eu,proof.Z_u);//EX6
    Fp12::pow(tempr6,ev,proof.Z_v);//EX7

    Fp12 t1,t2,t3,t4;
    Fp12::mul(t1,inv_tempr1,tempr2);//MP13
    Fp12::mul(t2,t1,tempr3);//MP14
    Fp12::mul(t3,t2,tempr4);//MP15
    Fp12::mul(t4,t3,tempr5);//MP16
    Fp12::mul(er,t4,tempr6);//MP17
    ...
    Fp12 S_c,lS,rS;
    Fp12::pow(S_c,proof.S,proof.c);//EX8
    Fp12::mul(lS, S_c, proof.Cm_S);//MP18

    Fp12::pow(rS,H ,proof.Z_v);//EX9
    ...
    Fp12::pow(D_c,proof.D,proof.c);//EX10
    Fp12::mul(lD, D_c, proof.Cm_D);//MP19

    Fp12::pow(temp_rD1,G ,proof.Z_u);//EX11
    Fp12::pow(temp_rD2,H1 ,proof.R);//EX12
    Fp12::pow(temp_rD2,temp_rD2 ,proof.Z_v);//EX13
...
bool Payment(User& payer, User& payee,
    ...
    pk_3_proof pk3 = pk_3_prove(payee, g, g0, g1, g_G2, G, INFO);//7MP+8EX+6BP
    //payee side 1 end
    ...
    //payer side 1 start
    // Step 3: SPK proof 생성
    SPK_proof spk = SPK_prove(payer, h, h0, h1, h2, h_G2, G, H, H1, INFO, pk3);//9MP+15EX+6BP
    //payer side 1 end

    // Step 4: SPK 검증
    //payee side 2 start
    if (!SPK_verify(spk, h, h0, h1, h2, h_G2, G, H, H1)) {//19MP+13EX+8BP
    ...
    const mcl::bn::G1 &h, const mcl::bn::G1 &h0, const mcl::bn::G1 &h1, const mcl::bn::G1 &h2, const mcl::bn::G2 &h_G2, const mcl::bn::Fp12 &H, const mcl::bn::Fp12 &H1)
{
    //payee side 1 start
    // Step 0: 검증 전 증명 생성
    pk_3_proof pk3 = pk_3_prove(user, g, g0, g1, g_G2, G,user.INFO);//7mp+8ex+5bp
    //payee side 1 end
    //bank side 1 start
    if (!pk_3_verify(pk3, g, g0, g1, g_G2, G)) {//16mp+7ex+7bp
    ...
    if (!SPK_verify(user.spk, h, h0, h1, h2, h_G2, G, H, H1)) {//19mp+13ex+8bp
        std::cout << "❌ Verification failed: SPK is not valid" << std::endl;
        return false;
    }
    ...
    pk_4_proof pk4 = pk_4_prove(user,tp,  vp, h0, h1, h2, Cmp, G);//3mp+2ex
    //payee side 2 end

    //bank side 2 start
    if (!pk_4_verify(pk4, Cmp, h0, h1, h2, G)) {//5mp+2ex
        std::cout << "❌ Verification failed: Pk_2 is not valid" << std::endl;
        return false;
    }
    ...
    G1 h0t, h2v, h0th1u, h0th1uh2v, hh0th1uh2v;
    G1::mul(h0t, h0, user.t);//mp4
    G1::mul(h2v, h2, user.v);//mp5
    G1::add(h0th1u, h0t, h1_u);
    G1::add(h0th1uh2v, h0th1u, h2v);
    G1::add(hh0th1uh2v, h0th1uh2v, h);

    G2 X, hf, Xhf;
    G2::mul(X, h_G2, beta);//mp6
    G2::mul(hf, h_G2, f);//mp7
    G2::add(Xhf, hf, X);
...
pk_5_proof pk_5_prove(User &user_out,const mcl::bn::Fp12 &G) {
    ...
    Fp12::pow(proof.Cm_U,G,r_u);//EX1

    std::string input = proof.INFO + proof.N.getStr(10);
    Fp12 G_t_info;
    hashToGT(G_t_info,input);

    Fp12::pow(proof.M,G_t_info,user_out.u);//EX2
    
    Fp12::pow(proof.Cm_M,G_t_info,r_u);//EX3
...
bool pk_5_verify(const pk_5_proof &proof,const mcl::bn::Fp12 &G) {
    ...
    Fp12 U_c,lU;
    Fp12::pow(U_c,proof.U,proof.c);//ex1
    Fp12::mul(lU, U_c, proof.Cm_U);//mp1
    Fp12 rU;
    Fp12::pow(rU,G,proof.Z_u);//ex2
    ...
    Fp12 M_c,lM;
    Fp12::pow(M_c,proof.M,proof.c);//ex3
    Fp12::mul(lM, M_c, proof.Cm_M);//mp2

    Fp12 rM;
    Fp12::pow(rM,G_t_info ,proof.Z_u);//ex4
...
//payee side 1 start
pk_5_proof pk5 = pk_5_prove(user,G);//3ex
//payee side 1 start

//bank side 1 start
if (!pk_5_verify(pk5, G)) {//2mp+4ex
